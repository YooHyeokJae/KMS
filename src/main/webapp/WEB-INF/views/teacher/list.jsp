...
<div class="container">
    ...
    <nav aria-label="Page navigation example" class="d-flex align-items-center">
        <ul class="pagination">
            <li class="page-item"><a class="page-link" href="<c:url value="/teacher/list?page=${pageStart-1}"/>">이전</a></li>
            <c:forEach var="i" begin="0" end="${pageBlock-1}">
                <li class="page-item<c:if test="${pageStart + i eq currentPage}"> active</c:if>"><a class="page-link" href="<c:url value="/teacher/list?page=${pageStart + i}"/>">${pageStart + i}</a></li>
            </c:forEach>
            <li class="page-item"><a class="page-link" href="<c:url value="/teacher/list?page=${pageStart+pageBlock}"/>">다음</a></li>
        </ul>
